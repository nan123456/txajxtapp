...
//	
	...
	if($wxylx=='quanbu' or $wxylx=='全部' ){
		$sql = "select * from 危险源  where 工程id='".$gcid."' and 工程名称='".$gcmc."'  order by 超过一定规模的危险性较大的分部分项工程  desc";
		...
		$sql = "select * from 危险源  where 危险源类型='".$wxylx."' and 工程id='".$gcid."' and 工程名称='".$gcmc."' order by 超过一定规模的危险性较大的分部分项工程  desc ";
		$result = $conn->query($sql);
		$class = mysqli_num_rows($result); 
		if ($result->num_rows > 0) { 
		while($row = $result->fetch_assoc()) {
