...
//			list($year, $month, $day) = explode ('-', $date); 
		...
		?>	</o:p>
           ...
           require("../../conn.php");
		   $tzdbh1=$_GET["tzdbh1"];
           $sql = "select * from 处罚条目 where 通知单编号='$tzdbh1'";
           $result = $conn->query($sql);
           while($row = $result->fetch_assoc()) { 
	  		$row["内容"]; 
...
<o:p><?php
           require("../../conn.php");
		   $tzdbh1=$_GET["tzdbh1"];
           $sql = "select * from 预览数据表 where 通知单编号='$tzdbh1'";
           $result = $conn->query($sql);
           while($row = $result->fetch_assoc()) { 
//			echo str_replace("||","。</br>",$row["内容"]);
            echo $row["项目部回复"];
		...
		<?php
           require("../../conn.php");
		   $tzdbh1=$_GET["tzdbh1"];
           $sql = "select * from 图片附件 where 通知单编号='$tzdbh1'";
           $result = $conn->query($sql);
           while($row = $result->fetch_assoc()) { 
...
<div class="row">
	<div id="imgCreat" class="col-md-4 col-md-offset-2">		
		<img src="../upload/<?php echo $hello[$index];?>"  class="col-md-6" alt="" style="width:300px;height:300px;/>
	</div>


	<div class="col-sm-4 col-md-offset-5" id="imgCreatHf">
		<img src="../upload/<?php echo $hello1[$index];?>" class="col-md-6" alt="" style="width:300px;height:300px;"/>  
	</div>
