<?php
	...
	if($flag=="yqsq"){
		...
		if($sjc){
			...
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
				/*20170619飞机   将通知单状态由未完成改为整改中*/
				$sqli = "update 通知单 set 通知单状态='整改中',是否延期='$biaozhi' where 时间戳='".$sjc."'";
				if ($conn->query($sqli) === TRUE) {
					$jsonresult='success';
				} else {
					$jsonresult='error';
		...
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	}else if($flag=="canceldelay"){
		//撤销延期
		$sql = "select * from 通知单 where id='".$biaozhi."'";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
			$sqli = "update 通知单 set 是否延期='',通知单状态='未完成' where id='".$biaozhi."'";
			if ($conn->query($sqli) === TRUE) {
				$jsonresult='success';
			} else {
				$jsonresult='error';
