<?php
	...
	if($flag=="yuqi"){
			...
			if($company=="总部"){
				$sqldate="";
				$sql = "select a.id,a.工程名称,b.通知单状态,b.通知单编号,b.整改期限,b.时间戳,b.工程id from 我的工程 a,通知单 b where b.通知单状态 ='整改中' and a.工程名称=b.工程名称    and b.检查层级 like '%".$company."%' and DATE_FORMAT(NOW(),'%Y-%m-%d 00:00:00')>整改期限 AND a.id=b.工程id and (总公司批复意见 ='' or 总公司批复意见 ='待总公司批复') order by a.工程名称,b.通知单状态 desc";
...
//				}else{
					$sql = "select a.id,a.工程名称,b.通知单状态,b.通知单编号,b.整改期限,b.时间戳,b.工程id from 我的工程 a,通知单 b where b.通知单状态='整改中' and a.工程名称=b.工程名称    and b.检查单位 = '".$section."' and DATE_FORMAT(NOW(),'%Y-%m-%d 00:00:00')>整改期限 AND a.id=b.工程id and 分公司批复 ='' order by a.工程名称,b.通知单状态 desc";
//				}
...
//				$sql = "select a.id,a.工程名称,b.通知单状态,b.通知单编号,b.整改期限,b.时间戳,b.工程id from 我的工程 a,通知单 b where b.通知单状态 in ('延期','整改中') and a.工程名称=b.工程名称    and b.检查层级 ='项目部自检'  order by a.工程名称,b.通知单状态 desc";
				$sql = "select a.id,a.工程名称,b.通知单状态,b.通知单编号,b.整改期限,b.时间戳,b.工程id from 我的工程 a,通知单 b where b.通知单状态 in ('延期','整改中') and a.工程名称=b.工程名称    and b.检查层级 ='项目部自检' and DATE_FORMAT(NOW(),'%Y-%m-%d 00:00:00')<整改期限 AND a.id=b.工程id  order by a.工程名称,b.通知单状态 desc ";
					...
					$sql = "select a.id,a.工程名称,b.通知单状态,b.通知单编号,b.整改期限,b.时间戳,b.工程id from 我的工程 a,通知单 b where b.通知单状态 in ('延期','整改中') and (a.时间戳 in(SELECT 工程时间戳 FROM `工程管理人员` WHERE 部门='项目部' AND 联系方式 = '".$mobile."' ) OR (a.联系方式='".$mobile."' OR a.项目经理手机='".$mobile."' OR a.安全主管手机='".$mobile."' OR a.`安全员手机`='".$mobile."'OR a.生产经理手机='".$mobile."' OR a.机械管理员手机='".$mobile."'OR a.质量员手机='".$mobile."')) and b.`检查层级` = '项目部自检' and DATE_FORMAT(NOW(),'%Y-%m-%d 00:00:00')<整改期限   AND a.id=b.工程id order by a.工程名称,b.通知单状态 desc";
				}
